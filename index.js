...
function CemberinCevresi(aYariCap) {
  return 2 * pi * aYariCap;
}

/* (Oto test yok) Yukarıdaki CemberinCevresi fonksiyonunu yarıçap = 5 vererek aşağıda çalıştırıp, sonucu konsolda gözlemleyin (console.log)  */

console.log("Çemberin Çevresi (5): ", CemberinCevresi(5));
...
function CemberinAlani(aYariCap, aPi) {
  return aPi * Math.pow(aYariCap, 2);
}
...
/* 	GÖREV 3:
		...
		oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
			3a. enbuyuk ve enkucuk isminde 2 adet değişken tanımlayın ve sayilar dizisindeki en küçük sayı ile en büyük sayıyı bu değişkenlere 
			...
			3c. `ucetambolunenler` dizisindeki sayıların toplamını .reduce metoduyla bulup, sonucu `ucebolunenlerintoplami` değişkenine 
			yazdırın (.reduce metodunu kullanın)
			...
			3e. besyuzdenkucuksayilar dizisindeki sayıları küçükten büyüğe sıralayıp `siralisayilar` adındaki bir diziye aktarın (.sort 
				metodunu kullanın)
...
console.log("Sayılar dizisindeki sayı adedi: ", sayilar.length);
